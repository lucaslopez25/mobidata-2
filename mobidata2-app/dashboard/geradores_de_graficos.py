...
def gerar_lista_de_anos_disponiveis_distinct(model):
    anos_disponiveis = model.objects.values_list('ano', flat=True).distinct().order_by('ano')

    return anos_disponiveis

def gerar_grafico_linhas_simples_por_ano_mes_filtravel_por_ano(model, ano_selecionado, yaxis_titulo, titulo_grafico, yaxis_modelname):
    queryset = model.objects.filter(ano=ano_selecionado).order_by('ano', 'mes')
